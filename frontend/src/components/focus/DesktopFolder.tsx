...
const DesktopFolder = ({ folder, onOpenModal, dragState, onDragStateChange }: DesktopFolderProps) => {
  ...
  const labelGaps = focusStore.desktopFolderLabelGaps ?? {};
  const updateLabelGap = focusStore.updateDesktopFolderLabelGap;
  ...
  const labelGap = labelGaps[folder.id] ?? 4;
      ...
      {contextMenu && createPortal(
        <>
          ...
          >
            {/* Drag handle bar */}
            <div
              className="flex items-center justify-center py-1 cursor-move border-b border-border/20 bg-secondary/30 hover:bg-secondary/50 transition-colors group"
              onPointerDown={(e) => {
                e.stopPropagation();
            ...
            <div className="px-4 py-2 border-b border-border/30 flex items-center gap-3">
              ...
              <input type="range" min="0" max="16" step="1" value={labelGap}
                onChange={(e) => { e.stopPropagation(); updateLabelGap(folder.id, parseInt(e.target.value)); }}
                onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}
                className="flex-1 h-1 rounded-full appearance-none bg-secondary cursor-pointer accent-primary"
              />
