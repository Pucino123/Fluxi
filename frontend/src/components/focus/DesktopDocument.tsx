...
const DesktopDocument = ({ doc, onOpen, onDelete, onDuplicate, onRefetch }: DesktopDocumentProps) => {
  ...
  const labelGap = store.desktopDocLabelGaps?.[doc.id] ?? 4;
    ...
    <>
      <motion.div
        ...
        animate={{
           ...
           * - Document being dragged dims to 0.35 opacity
           * - NO glow or ring on the dragged item itself
           * - The floating overlay (DragOverlay) shows the lifted state
           */
          opacity: isBeingDragged ? 0.35 : 1,
          scale: isBeingDragged ? 0.95 : 1,
        }}
        transition={{ type: "spring", stiffness: 400, damping: 25 }}
        className="desktop-folder absolute flex flex-col items-center justify-center p-2 pb-1 select-none rounded-2xl group cursor-grab hover:scale-[1.02] transition-transform duration-100"
        style={{ 
          left: pos.x, 
          top: pos.y, 
          width: 90, 
        ...
        <>
          ...
          >
            ...
            <div className="px-4 py-2 border-b border-border/30 flex items-center gap-3">
              <p className="text-[10px] text-muted-foreground uppercase shrink-0 flex items-center gap-1">↕ Gap</p>
              <input type="range" min="0" max="16" step="1" value={labelGap}
                onChange={(e) => { e.stopPropagation(); store.updateDesktopDocLabelGap(doc.id, parseInt(e.target.value)); }}
                onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}
                className="flex-1 h-1 rounded-full appearance-none bg-secondary cursor-pointer accent-primary" />
              <span className="text-[10px] text-muted-foreground tabular-nums w-7 text-right">{labelGap}px</span>
            </div>
